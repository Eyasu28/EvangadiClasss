...
const getEvens = x =>{
  ...
  console.log(getEvens([1, 2, 3, 6, 4, 8]));
  ...
  const twoDigit = x =>{
      ...
      return parseInt(x.toString().charAt(0)) > parseInt(x.toString().charAt(1)) ? 'ok' : 'Not ok';
  }
  ...
  const fact = (x) => {
    if (x < 0 || isNaN(x) || !(Number.isInteger(x))) return "Check your number";
    if (x < 2) return 1;
    return x * fact(x - 1);
  }
  console.log(fact(0)+ "\n\n");
  ...
  const meraArray = x =>{
      return new Set(x.concat(x.map(y => y*2))).size !=  x.concat(x.map(y => y*2)).length ? "I am NOT a Meera array" : "I am a Meera array"
  }
  ...
  const digitalClock = sec =>{
    ...
    let [h, m, s] =[('0' + Math.floor(sec/3600)).slice(-2),('0' + Math.floor((sec%3600)/60)).slice(-2), ('0' + Math.floor(sec%60)).slice(-2)]
    if(h == 24)  h="00";
    return [h,m,s]
  }
  let seconds=87000
  console.log(digitalClock(seconds)[0] + ":", digitalClock(seconds)[1] + ":", digitalClock(seconds)[2] );
